        ...
        ObjectContainer conexion = bd.getConexion();
            ...
            return conexion.query(new Predicate<Cliente>() {
                public boolean match(Cliente c) {
                    ...
                    if (clienteId != null && !c.getIdCliente().equals(clienteId)) return false;
                    if (nombre != null && !c.getNombre().equalsIgnoreCase(nombre)) return false;
                    if (ap1 != null && !c.getApellidoPaterno().equalsIgnoreCase(ap1)) return false;
                    if (ap2 != null && !c.getApellidoMaterno().equalsIgnoreCase(ap2)) return false;
                    if (telefono != null && !c.getTelefono().equals(telefono)) return false;
                    if (correo != null && !c.getCorreo().equalsIgnoreCase(correo)) return false;
